...
from Config.VAC.Meas_VAC import ExcelWriterThread
from Config.VAC.Instruments_VAC import InstrumentConnection, ConnectionThread
from Config.VAC.Create_Excel_Vac import Create_Open_Excel
...
class App(QMainWindow):
    ...
    def __init__(self):
        ...
        self.take_volt.clicked.connect(self.take_voltage_from_b4)
...
4. Запускайте измерения
    ...
    def take_voltage_from_b4(self):

        if self.ws is None:
            return

        last_row = self.ws.range('B4').end('down').row
        print("last_row: ", last_row)

        data_range = self.ws.range(f'B4:B{last_row}')
        self.vac_voltages = data_range.value

        print(self.vac_voltages)
    ...
    def pause(self):
        if self.animation_timer.isActive():
            ...
            self.start_button.setText("Старт ВАХ")
            self.animation_num = 2
