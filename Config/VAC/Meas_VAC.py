...
class ExcelWriterThread(QThread):
    ...
    def vac_cycle(self):
        ...
        for i in range(self.app_instance.vac_voltages):
            ...
            self.control_heater(channel=self.settings["ch_ip2"],
                                voltage=self.app_instance.vac_voltages[i],
                                state="on")
    ...
    def vac_res_step(self):
        ...
        for i in range(len(r5)):
            self.update_excel_signal.emit(self.number, start_col, r5[i])
            start_col += 1
        for i in range(len(r6)):
            self.update_excel_signal.emit(self.number, start_col, r6[i])
            start_col += 1
